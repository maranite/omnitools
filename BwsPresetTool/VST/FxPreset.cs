...
namespace BwsPresetTool.VST
{
    public class FxPreset : IEquatable<FxPreset>
    {
        ...
        public string PluginCode
        {
            get
            {
                return Encoding.ASCII.GetString(BitConverter.GetBytes(PluginID).Reverse());
            }
            set
            {
                PluginID = BitConverter.ToUInt32(Encoding.ASCII.GetBytes(value).Reverse(), 0);
            }
        }
        ...
        public static FxPreset From(string filePath)
        {
            using (var f = File.OpenRead(filePath))
                return From(f);
        }

        public static FxPreset From(Stream source)
        {
        ...
        public void ToFXB(BinaryWriter writer)
        {
        ...
        public byte[] ToFXB() => ToBytes(ToFXB); 

        public void ToFXP(BinaryWriter writer)
        {
            writer.WriteBE(CCnkMagic);                                          // long chunkMagic;   
        ...
        public byte[] ToFXP() => ToBytes(ToFXP);

        public void ToPCHK(BinaryWriter writer)
        {
        ...
        public byte[] ToPCHK() => ToBytes(ToPCHK);

        static byte[] ToBytes(Action<BinaryWriter> doWriting)
        {
